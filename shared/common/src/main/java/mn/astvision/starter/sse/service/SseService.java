package mn.astvision.starter.sse.service;

import lombok.extern.slf4j.Slf4j;
import mn.astvision.starter.sse.dto.SseRequest;
    ...
    public void sendEmitterSignal(final SseRequest<List<String>> sseRequest) throws IOException {
        ...
        try {
            for (int i = 0; i < totalPercentage; i++) {
                Thread.sleep(500);
                sseEmitter.send(SseEmitter.event().name(channelName).data(sseRequest.getData()));
