...
public class BookTypeApi extends BaseController {
    ...
    public ResponseEntity<?> create(
            @RequestBody BookType request,
            @AuthUser User user
    ) {
        if (ObjectUtils.isEmpty(request.getName()))
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(request.getCode() + request.getName() + " утга Хоосон байна.");

        try {
            request.setCreatedBy(user.getId());
            request.setCreatedDate(LocalDateTime.now());

            return ResponseEntity.ok( bookTypeRepository.save(request));
        } catch (Exception e) {
